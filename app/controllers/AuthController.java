...
public class AuthController extends Controller {
    ...
    public Result register() {
        ...
        if (regForm.hasErrors()) return badRequest(register.render(regForm));
            ...
            User user = new User(reg.email, reg.password, reg.name, false);
            user.save();
   ...
   /* public static Long currentUserId() {
        return Long.parseLong(session("id"));
    }
*/
    /**
     *
    ...
    public Result changePass() {
        Form<ChangePassword> changeForm = formFactory.form(ChangePassword.class).bindFromRequest();
        if (changeForm.hasErrors()) return badRequest(change.render(changeForm));
        else {
            ChangePassword change = changeForm.get();
            User user = currentUser();
            user.setPassword(change.newPassword);
            user.save();
            return redirect(routes.HomeController.index());
        }
    }
